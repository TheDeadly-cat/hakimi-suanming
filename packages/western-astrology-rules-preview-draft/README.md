...
结果区新增 SVG 星盘轮（`src/browser-app/chart-wheel.ts`）：黄道十二宫环、十二宫头环、天体落点与相位连线，0° 位于左侧、随经度逆时针递增；恒星黄道为演示选项，勾选后只使用 `manual_offset_unverified / subtract_supplied_offset_v1` 表示“减去调用方手填值”的工程身份。天体、宫头、四轴与轮盘共用规则层唯一黄道变换；这不计算或冒充 Lahiri 等命名岁差体系。轮盘几何是纯函数，可复用于未来紫微/西洋融合视图。
...
v0.8 不增加新星历、落位或动态组合规则，而是把可复用的内容基础拆成固定 43 项审稿队列，并以新的 template 身份绑定当前内容层。它取代仍绑定旧内容层的 v0.7 与 v0.6 候选；旧文件必须失败关闭，且所有版本都不获得正式激活或专家权威：
...
页面可下载 `hakimi-western-content-primitives-review-v008.json`。模板严格绑定当前有序 43 项快照、
...
v0.10 为当前成功排出的 `WesternContentProjection` 建立与 43 项基础审稿完全独立的逐卡反馈包，并把热带或手填恒星偏移的工程方法身份设为必填投影绑定。它以新的 template 身份绑定边界规范化后的内容层，取代 v0.9；缺少方法字段的 v0.8 与仍绑定旧内容层的 v0.9 都必须失败关闭，且各版本都不产生正式激活或专家权威。
...
页面只有在当前盘成功生成后才允许下载 `hakimi-western-current-chart-review-v010.json`。模板只接收
内容投影，不接收规则请求或表单，明确排除姓名、UTC、地点、`inputLabel`、RAMC、纬度、黄赤交角、
岁差数值与表单原值；模板保留非个人的 `kind / ayanamshaId / algorithmId` 工程身份，但仍携带精确落位、宫位、相位、守护链和事实摘要，所以只是“移除直接身份字段”，
