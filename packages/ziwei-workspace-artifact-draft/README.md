...
2026-08-31 的 Z-EGRESS-01 已把当前 4218 正文显示链上的 13 个来源 surface 与一层 browser-display projection 接入内存 fail-closed expression gate，即 **14/16** 个机械 surface。map 同时以私有品牌绑定 exact projection 对象、key 与 raw leaf；计算／重开的异步 map 完成后重查 `viewEpoch` 才渲染。两个 feedback/file preflight surface、`review_guardrail`、完整 ARIA/dataset/download sink、callgraph 与语义覆盖仍未闭合，不能称 16/16 或生产统一 egress。修复后的定向 Vitest **4 files / 80 tests**、两个 Browser strict TypeScript 和直接隔离构建 **110 modules transformed** 通过；本地 4218 只执行未保存计算并遍历十二宫，日志为 0，但默认盘没有可见 neutral fallback 命中。正式 build 脚本仍被既有全局 boundary 红门阻断，domain manifest 继续 `MANIFEST_MISMATCH`，没有重签。完整证据与剩余停止线见 `docs/其他体系独立工程增量-紫微西洋吠陀-2026-08-31.md`。

同日 Z-EGRESS-02 只把 v0.10 四化反馈与 v0.13 当前盘反馈中枚举的人类叙事字段接入各自 feedback-preflight surface，再以第一层 `displayText` 进入 browser-display；原始模板／下载、reviewer 与文件元数据、URL 文本、固定结构字段和 guardrail 仍不在 expression map 内。两条导入均在异步读取与 view 创建后重查 token，当前盘另绑定 exact projection／scope／`viewEpoch`；所有叙事和 HTTPS 导航先在 detached fragment 外失败关闭，再一次提交 live DOM。原 reviewer 与导入文件名不显示，URL 拒绝账号信息并固定 `noopener noreferrer`。定向 Vitest **4 files / 58 tests**、两个 strict TypeScript、锁定 Vite 7.3.6 隔离构建 **110 modules transformed**、Edge／Chrome 两场景各 **2/2（共 4/4）**和应用内 Browser safe／neutralized／credential-URL 三类导入通过；可见档案计数全程为 `0 / epoch 5 / 0 B`，但未执行 IndexedDB 六项 probe。现在只能说 16 个注册 surface ID 在指定枚举链路中均有受测调用点，不能说 16/16 sink/callgraph/semantic closure；正式 admission、专家、权利、发布与公开授权仍全部关闭。完整分账见同一证据文档。
