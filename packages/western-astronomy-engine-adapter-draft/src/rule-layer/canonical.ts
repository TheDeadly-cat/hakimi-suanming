...
export function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const t1 = (h + bigS1 + choose + SHA256_K[index]! + words[index]!) >>> 0;
...
export function sha256Hex(input: Uint8Array): string {
  return [...sha256Bytes(input)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
}
...
export function signedSeparationDeg(fromDeg: number, toDeg: number): number {
  return normalizeLongitudeDeg(toDeg - fromDeg + 180) - 180;
}

export function rad(valueDeg: number): number {
...
export function nearlyEqual(left: number, right: number, tolerance = 1e-7): boolean {
