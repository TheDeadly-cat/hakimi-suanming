...
当前日常使用的是已验证的 **c15ef05bb165 / legacy-v13 / Schema 13** 产物，入口为 **http://127.0.0.1:5188/**。已经安装的用户继续使用桌面的“哈基米八字研究台（本地自用）”。新源码、旧 `dist/web` 和这份固定产物分别记录；拉取代码不表示应替换安装版本。
...
先准备 Windows 自带 PowerShell 5.1、**Node.js 24.16.0** 和明确来源的固定产物安装包；不需要项目源码、npm依赖或本机旧目录。解压安装包后运行：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\install-local-research.ps1
```

安装器逐项核对完整清单和原产物锁，复制到当前用户的 `%LOCALAPPDATA%\HakimiBaziWorkbench\packages\c15ef05bb165`，创建带 `(5188)` 标记的快捷方式。重复安装只复用完全一致的包；不同目标内容或同名其他快捷方式会被保留并报告冲突。启动不构建、不换端口、不终止未知服务。Node未安装时需先安装规定版本，脚本不会联网下载工具。

源码维护者也可明确传入安装包路径：

```powershell
npm run desktop:install -- -PackageRoot "D:\Packages\hakimi-c15ef"
...
安装不下载依赖、不构建、不迁移案例；默认 `desktop:install` / `desktop:launch` 仍选择原 c15ef 日常产物。新修复包的独立安装，以及安装副本在 Edge/Chrome 中的启动、PWA、完整研究与备份恢复流程已通过。第二台 Windows 和本人研究反馈仍待完成，现有 5189 入口尚未切换。
...
2026-08-10 新增并关闭隔离 v13→v16 候选的当前 Chromium 故障矩阵；2026-08-11 在当前源码复跑为 Edge **13/13（约 5.3 分钟）**、Chrome **13/13（约 5.1 分钟）**，共 **26/26**。矩阵覆盖“双旧 v13 页，一页已 `BOOT_OK`、另一页仍在 10k 慢审计时 v16 worker 接管”的自然升级收敛；已确认旧页被拖过 5 秒冻结时限时 v16 试运行页自动重试并完成收敛（新增确定性场景）；以及全新首装、富数据、跨页写锁、容量不足、目标启动/完整性审计失败、Dexie 事务中止、`BOOT_OK` 中断、陈旧 target、dirty 审计期间 CAS 竞争、同 ID 终态隔离和新 ID 长期重发。新 worker 对仍未绑定目标代的合法旧页返回明确的代际收敛信号；旧页保持 `inert` 和写锁，自动 reload 后只绑定已提交 v16，不把旧 `BOOT_OK` 当成目标确认。失败路径保持 v13 源不变、目标隔离或零创建，不产生可写半代；源冻结失败做最多 5 次、2～6 秒退避的有界重试，失败页不再错误停留在 `pending`，影子启动预算为 300 秒（仍是有界失败关闭）。

`test:e2e:web-v1-flow:v13-to-v16` 在 2026-08-10 于 Edge 与 Chrome 各通过 1/1、共 2/2（约 2.5 分钟），把十六个用户分区全部非空、Revision 计算收据、断网报告、全量清空、全新 context 恢复和稳定二次导出放入同一候选流程（2026-08-11 因本机持续高负载未能在 45 分钟内完成复跑，保留历史证据）。这里的工程命中、恢复与哈希一致只证明软件契约，不增加命理金标或专家验证计数。v16 仍是隔离候选，普通构建和 `dist/web` 继续为 `legacy-v13`。
...
`test:e2e:orphaned-v13-recovery` 已关闭“没有已确认旧 Service Worker 壳、没有 release-control、且浏览器中只存在一个结构精确匹配的 v13 源库”的 R0 只读恢复门：Edge 151 与 Chrome 150 各通过 1/1，共 2/2。恢复页全程没有请求 `/sw.js`，没有注册或取得 Service Worker 控制权，也没有创建 control、v14、v15 或 v16 数据库；源 v13 的原生快照在导出前后逐项不变。下载得到的 full v1.2 ZIP 又在隔离的 v13 context 中通过十六分区只读预检。只要库存出现多库、结构不精确或状态竞态等歧义，页面就只显示脱敏诊断并失败关闭，不自动迁移、恢复、删除或导出。Firefox、固定 Android Chrome/WebView、唯一生产升级谱系确认、候选产物验签与任何默认代提升仍未完成。完整边界见 [Revision 下游计算收据与复演协议 v0.1](./docs/Revision下游计算收据与复演协议-v0.1.md)。当前工程总账以 Vitest 146 文件 / 1537 项（2026-08-11 `--maxWorkers=2` 复跑）、v13→v16 Edge + Chrome 各 13/13（2026-08-11 共 26/26）、v13→v16 连续 Web 流程 2/2（2026-08-10 记录）和 clean-start 双浏览器门（2026-08-11 复验）为准；历史 v14/v15 数据继续作为冻结回归证据。

下游来源现由报告与 ResearchQuery 共同消费同一个严格 resolver。单盘报告当前为 `formatVersion=1.7.0`；但保存的 v1.7 domain manifest 因 D0 三处字节漂移尚未与当前工作区闭合，不能引用其旧摘要作为当前有效机器身份、发布就绪或授权。ResearchQuery 执行器为 `0.2.0`，查询快照导出为 `1.1.0`。真实账本精确命中标为 `stored_receipt`，无精确收据时标为 `explicit_projection`，即时投影本身不可执行时报告为 `not_evaluable`；关系、起运、Transit 逐组件保留 `projected`、`unavailable`、`not_requested`、`not_evaluable` 或对应比较状态。Schema 13/14 的账本状态明确为 `schema_unavailable`，不得冒充历史收据；Schema 15 也只有源 Revision、Profile、规范请求指纹唯一匹配时才使用收据，`matched`、`mismatch` 与 `exact_executor_unavailable` 另行报告。损坏、重复、歧义或来源失配失败关闭，绝不静默回退当前版本。匿名单盘保留来源类别与复演结论，但移除收据 ID、请求指纹、收据/投影/组件摘要和保存时间。
...
packages/bazi-interpretation 旺衰、十神、神煞的来源绑定候选、审稿清单与当前盘只读复核包；不自动激活吉凶
...
packages/research-export 精确单盘报告 v1.7.0（保留冻结 v1.3 历史证据）、下游来源与 Event 时间派生回执、匿名扫描/黄金契约，以及 Markdown/全案例 CSV 导出
...
packages/cross-system-comparison-draft v0.3 只读并列草案；只接受紫微／西洋当前离线工程回执绑定事实，无评分／投票／模型仲裁，生产导入禁止且正式比较未授权
```

`lunar-typescript@1.8.6` 只作为实验候选实现。产品不会直接暴露其宜忌/吉凶 API，也不会把上游输出宣称为已经通过本项目金标准的生产真值。

八字内容层现有 69 项全量审稿目录：4 个旺衰方法问题、`10 × 4 = 40` 条十神落柱候选、5 个神煞公式和 `5 × 4 = 20` 条神煞落柱候选。v0.17 将执行口径统一为“月令 4、透干 2、首位藏干 2、其余藏干 1”，并新增当前盘实际十神／神煞命中复核包；v0.18 又增加 8 个来源、12 个精确来源 binding 与 12 条旺衰 claim，按“月令→透干→首藏→余藏”显示逐项贡献、月主气 `4 + 2` 双计、五档区间与六个工程扰动。Wikisource 只按 pinned oldid／locator 提供传统语境，未核《渊海子平》和未冻结《子平真诠评注》对应主张保持 withheld；4/2/2/1、阈值与六情景只由本仓工程政策定义。所有投影仍不写数据库、不继承审稿决定，`goodBad/usefulGod/structure/event/result` 均关闭。v0.18 已完成定向 Vitest 4 files / 27 tests、默认身份 3 files / 23 tests、typecheck、草案隔离 verifier、应用内 Browser 桌面／390×844 和 Edge + Chrome 2/2；E2E build 为 2058 modules / `bc2c28f15407`，完整浏览器存储前后相等，默认发布合同仍为 `legacy-v13 / targetSchema 13 / migrationId null`。这些是定向收口证据，不代表整仓长回归、Android 或专家验收。详见[八字内容 v0.18](./docs/八字内容-v0.18-来源主张账与旺衰证据叙事-2026-08-13.md)与[八字内容 v0.17](./docs/八字内容-v0.17-旺衰政策单一事实源与当前盘命中复核包-2026-08-13.md)。

紫微工程探针可用上面的 demo 命令运行，也可单独构建并打开本机 `4216` 端口的浏览器十二宫预览。两条路径都为每次计算新建隔离 Worker；Browser Worker 返回包含 input、冻结规则、facts、provenance/evidence、执行身份和四层 canonical SHA-256 的专属工程工件。主线程重新验真工件与当前 19 个 TypeScript 文件及 1 个闭包 JSON 构成的源码图后，才从已验真的 facts/rule snapshot 本地派生宫位、星曜、亮度和四化显示；该工件不是、也不能冒充强制 Node runtime 的 fixture receipt。Fortel 差分会同时新鲜运行固定 iztro 与 Fortel Worker，再逐项比较公农历、命身宫、十二宫、14 主星、14 辅星、四化与大限；外部 JSON 的摘要自洽不再被当作引擎身份证明，报告的结构/摘要门也只承认内容自洽，只有 fresh 复现门会重跑两套引擎并返回本次新报告，历史 Worker ID 与时间戳不冒充已认证。月干支、时干支、亮度、辅助星全集和解释层保持不可比，晚子策略未对齐时直接失败关闭。它已稳定保留戊/庚/壬年化科表的具名差异，但不输出一致率、胜负或专家结论。
...
4218 的隔离内容层现已把十二宫问题域、十四主星基础与 `14 × 12 = 168` 条落宫候选、逐星／逐宫复核包、本命生年四化和 `4 × 12 = 48` 条四化落宫候选组织为当前盘输出。v0.11 为每个当前宫位生成“本宫主题—外部牵引—资源／压力观察—矛盾合成”四段式直读；v0.12 又冻结左辅、右弼、文昌、文曲、天魁、天钺、擎羊、陀罗、火星、铃星、地空、地劫十二个精确星键，形成 12 条基础候选与 `12 × 12 = 144` 条落宫中性候选，并只为当前盘真实命中项展示。v0.13 源码合同进一步把当前整盘组织为 12 个目标宫组；标准盘十二个核心星曜事实各进入 `self / opposite(+6) / trine(+4) / trine(-4)` 四组，形成 48 个 occurrence 的动态离线审稿模板。五来源只复用既有星曜、宫位和几何材料，不新增星义语义来源；切宫只过滤显示，编辑或重算会令旧包失效。模板删除直接身份与原始输入但仍含敏感派生命盘事实，SHA-256 不是加密或签名。传统分组不等于个人吉凶；禄存、天马、天空、其他 minor 与全部 auxiliary 的两个候选字段继续返回 `null`，而星键／标签／类别错配、未知地支、星曜 × 地支亮度矩阵失配或非法四化才抛错失败关闭。所有方向、事件、结果、评分和正式激活仍为 `null / false`。v0.13 本轮新执行 adapter 定向 Vitest **2 files / 30 tests**、三个 typecheck、草案隔离门、默认身份定向 **3 files / 18 tests**、4218 构建 **108 modules transformed**、应用内 Browser，以及 Edge／Chrome 独立场景各 **1/1、共 2/2**；完整存储快照保持、390px 响应式和截图哈希均已核对，且没有借用 v0.12 数字。详细边界见 [紫微内容 v0.11](./docs/紫微内容-v0.11-当前盘三方四正四段式直读候选与证据-2026-08-13.md)、[紫微内容 v0.12](./docs/紫微内容-v0.12-核心十二辅煞星基础与十二宫144条候选证据-2026-08-13.md)与[紫微内容 v0.13](./docs/紫微内容-v0.13-核心十二辅煞三方四正命中复核包与证据-2026-08-14.md)。

西洋 Astronomy Engine 探针只接受规范 UTC，输出地心 EQJ、真黄道日期位置与 UTC ±60 秒中心差分，并明确标记 Moon 的直接 `GeoMoon` 语义；Browser/Node 隔离页在 Edge、Chrome 分别重放五个 fresh Worker 种子，以版本化十进制稳定投影逐字段核对，同时保留 raw canonical 差异。它仍只返回 `diagnostic_only` 信封，不生成严格星盘或 `western-calculation-receipt`；当前也没有取得可锁定的 JPL Horizons 官方响应字节，未用合成数据代替。规则层已另把诊断位置接入纯几何黄道/宫位/相位竖切（`npm run demo:western:rules`），输出仍固定 `astrology_rules_engineering_artifact` 且不产生回执；`4219` 规则层浏览器预览（`npm run preview:western-rules-preview`）每次新建一次性 Worker、只渲染不保存，带 SVG 星盘轮与恒星黄道演示，已在 Edge 151 与 Chrome 151 完成十体/十二宫/角点/相位核验，全程零存储、零外网请求。其隔离内容层已从 v0.2 的 50 项结构候选扩展到 v0.5 的命主星、定位星链、四轴距离、10 张逐星综合及日月上升命主星四步首读；当前盘候选总数为 `50 + 实际主要相位数`，默认样例为 73。v0.6 的 43 项基础审稿与 v0.7 的当前盘动态复核包相互隔离，换盘即失效、只读预检且全部保持 `result:null`。JPL Horizons 差分离线管道已就绪（`npm run demo:western:horizons`）：冻结 2025 春分太阳地心 VECTORS 查询清单、证据 SHA-256 校验与 raw AU 差分报告，官方字节缺失时固定失败关闭；2026-08-10 对 JPL 官方端点的 TLS 直连与代理尝试仍被网络层阻断，未用合成数据代替。所有这些隔离路径都不写生产数据库、不进入八字 full v1.2，也不会改变普通 `npm run build` 的 `legacy-v13 / targetSchema 13` 身份。隔离历法门另从 DATA.GOV.HK 的香港天文台 2023–2028 六份年度 CSV 离线还原 **2,192 日 / 74 个农历月界 / 5 个跨年接缝**，并只声明 `calendar_resolution`；`upstream_regression`、`differential_diagnostic`、摘要一致和恢复成功都只是固定依赖图与软件协议的工程证据，不是专家真值。
...
- [八字内容 v0.18](./docs/八字内容-v0.18-来源主张账与旺衰证据叙事-2026-08-13.md)：8 个来源、12 个精确 binding、12 条旺衰 claim、逐项证据叙事、pinned oldid／数字转录权利边界，以及定向测试、双浏览器与截图证据。
- [八字内容 v0.17](./docs/八字内容-v0.17-旺衰政策单一事实源与当前盘命中复核包-2026-08-13.md)：旺衰 4/2/2/1 单一政策账、当前盘实际命中复核包、来源分层与双浏览器零写入证据。
- [术数内容新目标与来源路线图](./docs/术数内容新目标与来源路线图-2026-08-13.md)：八字、紫微、西洋当前覆盖、竞品／典籍／工程来源链接、统一内容模型与 P1～P4 可执行目标。
- [紫微内容 v0.13](./docs/紫微内容-v0.13-核心十二辅煞三方四正命中复核包与证据-2026-08-14.md)：当前整盘 12 组三方四正、标准 48 occurrence、五来源动态审稿模板、敏感派生事实与失败关闭边界，以及定向单测、构建、应用内 Browser、Edge／Chrome、存储快照和截图证据。
