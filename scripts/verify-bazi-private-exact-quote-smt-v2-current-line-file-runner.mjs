...
const OK_PREFIX = "BAZI_SMT_V2_PRIVATE_FILE_RUNNER_OK";
const FAILED_PREFIX = "BAZI_SMT_V2_PRIVATE_FILE_RUNNER_FAILED";
const ROOT_ENVIRONMENT_NAME = "HAKIMI_BAZI_SMT_PRIVATE_ROOT";
...
export async function main() {
  ...
  try {
    ...
    } = await import("./bazi-private-exact-quote-smt-v2-current-line-file-runner-lib.mjs");
    try {
      const receipt = await verifyBaziSmtV2PrivateExactQuoteFromFixedPrivateRoot(privateRoot);
      if (!isVerifiedBaziSmtV2PrivateFileRunnerReceipt(receipt)) {
        fail("PRODUCTION_RECEIPT_BRAND_REQUIRED");
        return;
      }
      process.stdout.write(`${OK_PREFIX} ${JSON.stringify(receipt)}\n`);
    } catch (error) {
      fail(safeBaziSmtV2PrivateFileRunnerCliCode(error));
    }
  } catch {
