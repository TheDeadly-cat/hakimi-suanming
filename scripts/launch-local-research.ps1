...
try {
  if ([string]::IsNullOrWhiteSpace($PackageRoot)) {
    $PackageRoot = Join-Path $env:LOCALAPPDATA "HakimiBaziWorkbench\packages\c15ef05bb165"
  }
  if ([string]::IsNullOrWhiteSpace($NodeExecutable)) { $NodeExecutable = (Get-Command node.exe -ErrorAction Stop).Source }
  if ([string]::IsNullOrWhiteSpace($LogRoot)) { $LogRoot = Join-Path $env:LOCALAPPDATA "HakimiBaziWorkbench\local-launch-logs" }
  ...
  $cliPath = Join-Path $sourceRoot "scripts\local-research.mjs"
  $mutex = New-Object Threading.Mutex($false, "Local\HakimiBaziLocalResearch-5188")
  ...
  if ($LASTEXITCODE -ne 0) { throw "The fixed package did not pass verification. See verify.log." }
  $listeners = @(Get-NetTCPConnection -State Listen -LocalPort 5188 -ErrorAction SilentlyContinue)
  if (@($listeners | Where-Object { $_.LocalAddress -ne "127.0.0.1" }).Count -gt 0) {
    throw "Port 5188 is not loopback-only. No service was stopped and no port was changed."
  }
  ...
  $started = $false
  if ($probeCode -ne 0) {
    if ($probeCode -ne 2 -or $listeners.Count -gt 0) { throw "Port 5188 belongs to a different or unverified service. Startup stopped; the service was not terminated." }
    ...
    if ($probeCode -ne 0) { throw "The new listener did not return the fixed artifact. See diagnostic logs." }
  }
  $listeners = @(Get-NetTCPConnection -State Listen -LocalPort 5188 -ErrorAction SilentlyContinue)
  if ($listeners.Count -ne 1 -or $listeners[0].LocalAddress -ne "127.0.0.1") { throw "Expected exactly one loopback listener on 5188." }
  ...
  if (-not $NoDialogs) {
    ...
    [void][Windows.Forms.MessageBox]::Show($message, "Hakimi local research")
  }
  exit 1
} finally {
