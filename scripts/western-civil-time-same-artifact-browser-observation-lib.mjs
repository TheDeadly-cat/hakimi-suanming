...
export const WESTERN_SAME_ARTIFACT_CANDIDATE_PATH =
  "content/system-admission/western-civil-time-same-artifact-browser-observation-child.v1.0.0.json";
export const WESTERN_SAME_ARTIFACT_CHILD_ID =
  "hakimi.western.civil-time.same-artifact-edge-chrome-observation/1.0.0";
export const WESTERN_SAME_ARTIFACT_DRAFT_ROOT =
...
const CANDIDATE_DIGEST_DOMAIN =
  "hakimi.western.civil-time.same-artifact-browser-observation-child.v1\0";
const FILE_GRAPH_DIGEST_DOMAIN =
  "hakimi.western.civil-time.same-artifact-file-graph.v1\0";
const OUTPUT_TREE_DIGEST_DOMAIN =
  "hakimi.western.civil-time.same-artifact-output-tree.v1\0";
const SERVED_BODY_DIGEST_DOMAIN =
  "hakimi.western.civil-time.same-artifact-served-body-manifest.v1\0";
const CANDIDATE_BRAND = new WeakSet();

export const WESTERN_AUTHORED_BUILD_SOURCE_PATHS = Object.freeze([
  "package.json",
  "package-lock.json",
  "tsconfig.json",
  "tsconfig.base.json",
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/package.json`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/tsconfig.json`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/browser-app/index.html`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/vite.config.mjs`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/vite.same-artifact-evidence.config.mjs`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/styles.css`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/browser-fact-projection.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/civil-client.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/civil-time.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/civil-worker.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/main.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/protocol.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/ui-format.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/src/worker-fact-projection.ts`,
...
export const WESTERN_EVIDENCE_TOOL_PATHS = Object.freeze([
  "scripts/western-civil-time-same-artifact-browser-observation-lib.mjs",
  "scripts/run-western-civil-time-same-artifact-browser-observation.mjs",
  "scripts/render-western-civil-time-same-artifact-browser-observation-candidate.mjs",
  "scripts/verify-western-civil-time-same-artifact-browser-observation.mjs",
  "scripts/verify-western-civil-time-same-artifact-browser-observation.test.mjs",
  "scripts/verify-western-civil-time-same-artifact-config-cwd.test.mjs",
  "scripts/western-civil-time-fact-browser-observation-lib.mjs",
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/playwright.same-artifact-evidence.config.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/tsconfig.same-artifact-evidence.json`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/e2e/civil-time-browser-gate.spec.ts`,
  `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/e2e/same-artifact-summary-reporter.ts`,
...
export const WESTERN_FORMAL_CONTEXT_PATHS = Object.freeze([
...
export const WESTERN_E2E_ASSERTION_CONTRACT_BINDINGS = deepFreeze([
  {
    role: "playwrightConfig",
    path: `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/playwright.same-artifact-evidence.config.ts`,
    bytes: 5067,
    sha256: "3578a33e4cbbdcb02c01c1cb9e4f5632f59f0ed3ff00662e235d88fd62e80a22"
  },
  {
    role: "fiveScenarioSpec",
    path: `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/e2e/civil-time-browser-gate.spec.ts`,
    bytes: 9618,
    sha256: "1906e5c7a3066f26c677bfa0db37f84d3372149677d0ee9ffaa9f30f50a288fb"
  },
  {
    role: "sanitizedReporter",
    path: `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/e2e/same-artifact-summary-reporter.ts`,
    bytes: 9852,
    sha256: "d13252d76b529d97acfdf789170a6893173d727ea0fe23adebdd26b7b969119c"
  }
]);

export const WESTERN_LOCKED_BUILD_INPUTS = deepFreeze([
...
export const WESTERN_SCENARIOS = deepFreeze([
...
export class WesternSameArtifactObservationError extends Error {
  constructor(code, detail = "") {
    super(detail ? `${code}: ${detail}` : code);
    this.name = "WesternSameArtifactObservationError";
    this.code = code;
  }
}

function fail(code, detail = "") {
  throw new WesternSameArtifactObservationError(code, detail);
}
...
export function collectWesternAuthoredBuildSourceSnapshot(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const files = WESTERN_AUTHORED_BUILD_SOURCE_PATHS.map((entry) => identityForPath(resolvedRoot, entry));
  return deepFreeze({
    graphId: "hakimi.western.civil-time.authored-build-source-graph/1",
...
export function collectWesternEvidenceToolSnapshot(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const files = WESTERN_EVIDENCE_TOOL_PATHS.map((entry) => identityForPath(resolvedRoot, entry));
  return deepFreeze({
    graphId: "hakimi.western.civil-time.same-artifact-evidence-tooling/1",
...
export function collectWesternLockedBuildInputSnapshot(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const inputs = WESTERN_LOCKED_BUILD_INPUTS.map((expected) => {
  ...
  return deepFreeze({
    graphId: "hakimi.western.civil-time.base-config-locked-build-inputs/1",
...
function verifyWesternE2EAssertionContractBindings(bindings) {
  if (!Array.isArray(bindings) || bindings.length !== WESTERN_E2E_ASSERTION_CONTRACT_BINDINGS.length) {
    fail("E2E_ASSERTION_CONTRACT_INVALID");
  }
  for (let index = 0; index < bindings.length; index += 1) {
    const actual = bindings[index];
    const expected = WESTERN_E2E_ASSERTION_CONTRACT_BINDINGS[index];
...
export function inspectWesternE2EAssertionContract(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const bindings = WESTERN_E2E_ASSERTION_CONTRACT_BINDINGS.map((expected) => ({
    role: expected.role,
    ...identityForPath(resolvedRoot, expected.path)
  }));
  return deepFreeze({
    inspectionId: "hakimi.western.civil-time.same-artifact-exact-e2e-files/1",
    bindingCount: bindings.length,
    bindings: verifyWesternE2EAssertionContractBindings(bindings),
...
function parseStrictJsonBytes(bytes, label) {
  ...
  try {
    ...
    if (cause instanceof WesternSameArtifactObservationError) throw cause;
...
export function collectWesternFormalContextSnapshot(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const parsed = new Map();
  const files = WESTERN_FORMAL_CONTEXT_PATHS.map((entry) => {
  ...
  const civil = parsed.get(WESTERN_FORMAL_CONTEXT_PATHS[0]);
  const parent = parsed.get(WESTERN_FORMAL_CONTEXT_PATHS[1]);
  const source = parsed.get(WESTERN_FORMAL_CONTEXT_PATHS[2]);
  const manifest = parsed.get(WESTERN_FORMAL_CONTEXT_PATHS[3]);
  if (civil?.activeAdmissionEffect !== "none"
      || civil?.systemIdentity?.contractSystemId !== "western"
      || civil.systemIdentity.productSystemId !== "western-astrology"
      || civil.systemIdentity?.releaseIdentity !== null
      || civil.systemIdentity?.targetSchema !== null
      || civil.systemIdentity?.migrationId !== null
      || civil?.gateSummary?.bindingRequired !== 28
      || civil.gateSummary.bindingFrozenVerified !== 0
      || civil.gateSummary.independentExpertsRequired !== 2
      || civil.gateSummary.independentExpertReviewsVerified !== 0
      || civil.gateSummary.admissionGatesRequired !== 8
      || civil.gateSummary.admissionGatesSatisfied !== 0
      || civil.observationBoundary?.mutationEpochAvailable !== false
      || civil.observationBoundary.mutationEpochReceipt !== null) {
    fail("CIVIL_BASELINE_CONTEXT_INVALID");
  }
  if (parent?.activeAdmissionEffect !== "none"
      || parent?.gateSummary?.bindingRequired !== 28
  ...
  if (source?.schemaVersion !== "1.4.0"
      || source?.productSystemId !== "western-astrology"
      || source?.contractSystemId !== "western"
      || source?.gateSummary?.bindingRequired !== 28
      || source.gateSummary.bindingFrozenVerified !== 0
      || source.gateSummary.sourceBodiesBound !== 0
      || source.gateSummary.exactQuotesBound !== 0
      || source.gateSummary.workRightsEstablished !== 0
      || source.gateSummary.versionRightsEstablished !== 0
      || source.gateSummary.carrierRightsEstablished !== 0
      || source.gateSummary.rightsLegalConclusionEstablished !== false
      || source.gateSummary.redistributionAuthorized !== false
      || !SHA256.test(source.ledgerDigest ?? "")) {
    fail("SOURCE_CONTEXT_INVALID");
  }
  const serializedManifest = canonicalCompact(manifest);
  if (manifest?.activeAdmissionEffect !== "none"
      || manifest.status !== "current_three_package_authored_file_machine_identity_zero_admission_effect"
      ...
      || serializedManifest.includes(WESTERN_SAME_ARTIFACT_CANDIDATE_PATH)
      || serializedManifest.includes(WESTERN_SAME_ARTIFACT_CHILD_ID)) {
    fail("MANIFEST_CONTEXT_INVALID");
  }
  assertAllFalse(civil.authorityBoundary, [
    "astronomicalFactsEstablished", "canonicalZoneIdentityEstablished",
    "civilTimeDomainTruthCertified", "contentTruthEstablished", "crossSystemAuthorityInherited",
    "domainAuthorityAuthorized", "expertClaimsAuthorized",
  ...
  assertAllFalse(parent.authorityBoundary, [
    ...
    "highRiskClaimsAuthorized", "publicDeploymentAuthorized",
  ...
  assertAllFalse(manifest.authorityBoundary, [
    "contentTruthEstablished", "domainAuthorityAuthorized",
    "expertClaimsAuthorized", "expertTruthEstablished", "formalAdmissionAuthorized",
  ...
  return deepFreeze({
    ...
    currentSourceRequirementsLedgerDigest: source.ledgerDigest,
...
export function collectWesternOutputTree(outDir) {
  ...
  const files = walkOutputFiles(real).sort(compareCodeUnits).map((relativePath) => {
...
function validateOutputFileSet(files) {
  ...
  const paths = files.map((entry) => entry.path).sort(compareCodeUnits);
  ...
  const expected = [
    ...
    "hakimi-western-fact-only-build-manifest.v1.json",
    "index.html",
...
export function deriveWesternRequiredRuntimePaths(outputTree) {
  return deriveWesternScenarioRuntimePaths(outputTree);
}

export function deriveWesternScenarioRuntimePaths(outputTree) {
...
export function deriveWesternEvidenceOnlyOutputBodyProbePaths(outputTree) {
  const paths = outputTree.files.map((entry) => entry.path);
  return Object.freeze([
...
export function verifyWesternBuildManifest(outDir, outputTree, root = process.cwd()) {
  const manifestPath = path.join(path.resolve(outDir), "hakimi-western-fact-only-build-manifest.v1.json");
  const manifest = parseStrictJsonBytes(readStableFile(manifestPath, "build manifest"), "build manifest");
  const payload = outputTree.files.filter((entry) => entry.path !== "hakimi-western-fact-only-build-manifest.v1.json");
  const baseConfigIdentity = identityForPath(workspaceRoot(root), `${WESTERN_SAME_ARTIFACT_DRAFT_ROOT}/vite.config.mjs`);
  if (manifest?.schemaVersion !== "1.0.0"
      || manifest.recordType !== "hakimi_western_fact_only_temp_build_manifest_v1"
      || manifest.generatedBy !== "hakimi-western-fact-only-output-envelope/1"
      ...
      || manifest.concurrentSameAccountPathReplacementExcluded !== false
      || manifest.concurrentSameAccountConfigReplacementDuringLoaderWindowExcluded !== false
...
function normalizePlaywrightSummary(value) {
  ...
  if (value.schemaVersion !== "hakimi.western.same-artifact-playwright-summary/1"
  ...
  for (const projectName of ["chrome", "msedge"]) {
    for (const scenario of WESTERN_SCENARIOS) expected.push(`${projectName}\0${scenario.scenarioId}`);
  }
  const seen = new Set();
...
function normalizeServer(value, outputTree) {
  ...
  const requiredPaths = deriveWesternRequiredRuntimePaths(outputTree);
  const scenarioRuntimePaths = deriveWesternScenarioRuntimePaths(outputTree);
  const evidenceOnlyOutputBodyProbePaths = deriveWesternEvidenceOnlyOutputBodyProbePaths(outputTree);
  ...
  const normalizedFaviconProjects = faviconProjects.map((entry, index) => {
    ...
    if (entry.projectName !== ["chrome", "msedge"][index] || entry.responseCount !== 6) {
      fail("FAVICON_CONTROL_INVALID");
    }
    return { projectName: entry.projectName, responseCount: 6 };
  });
  if (faviconControl.requestMethod !== "GET" || faviconControl.canonicalPath !== "/favicon.ico"
      ...
      || faviconControl.aggregateResponseCount !== 12
  ...
  if (value.host !== "127.0.0.1" || value.scheme !== "http" || !LOOPBACK_ORIGIN.test(originString)
      ...
      || value.responseHeaderName !== "x-hakimi-western-output-tree-sha256"
  ...
  const normalized = projects.map((entry, index) => {
    ...
    const combinedFiles = [...runtimeFiles, ...probeFiles].sort((a, b) => compareCodeUnits(a.path, b.path));
  ...
  if (value.totalControlledHttpResponseCount !== value.servedResponseCount + 12) {
...
export function normalizeWesternRuntimeObservation(value) {
  requireKeys(value, [
  ...
  if (typeof value.nodeVersion !== "string" || !VERSION.test(value.nodeVersion)
      ...
      || value.transformedModuleCount !== 88
...
export function buildExpectedWesternSameArtifactCandidate(rootOrObservation, maybeObservation) {
  ...
  const resolvedRoot = workspaceRoot(root);
  const runtimeObservation = normalizeWesternRuntimeObservation(observation);
  const source = collectWesternAuthoredBuildSourceSnapshot(resolvedRoot);
  const tools = collectWesternEvidenceToolSnapshot(resolvedRoot);
  const locked = collectWesternLockedBuildInputSnapshot(resolvedRoot);
  const formalContext = collectWesternFormalContextSnapshot(resolvedRoot);
  const e2eInspection = inspectWesternE2EAssertionContract(resolvedRoot);
  ...
  const unsigned = {
    schemaVersion: "1.0.0",
    recordType: "western_civil_time_same_artifact_edge_chrome_browser_observation_child",
    childId: WESTERN_SAME_ARTIFACT_CHILD_ID,
    ...
    e2eAssertionContract: {
      contractId: "hakimi.western.civil-time.same-artifact-five-scenarios/1",
      scenarioCountPerBrowser: 5,
      expectedOutcomeCount: 10,
      scenarios: WESTERN_SCENARIOS,
...
export function serializeWesternSameArtifactCandidate(candidate) {
  return `${JSON.stringify(candidate, null, 2)}\n`;
}

export function verifyWesternSameArtifactCandidateObject(root, value) {
  const resolvedRoot = workspaceRoot(root);
  const captured = captureJsonData(value, "Western same-artifact candidate");
  const expected = buildExpectedWesternSameArtifactCandidate(resolvedRoot, captured.runtimeObservation);
...
export function loadWesternSameArtifactCandidate(root = process.cwd()) {
  const resolvedRoot = workspaceRoot(root);
  const candidateBytes = readStableFile(
    resolveWorkspacePath(resolvedRoot, WESTERN_SAME_ARTIFACT_CANDIDATE_PATH),
    WESTERN_SAME_ARTIFACT_CANDIDATE_PATH
  );
  const parsed = parseStrictJsonBytes(candidateBytes, WESTERN_SAME_ARTIFACT_CANDIDATE_PATH);
  const expected = verifyWesternSameArtifactCandidateObject(resolvedRoot, parsed);
  const expectedBytes = Buffer.from(serializeWesternSameArtifactCandidate(expected), "utf8");
  if (!candidateBytes.equals(expectedBytes)) fail("CANDIDATE_CURRENT_REBUILD_MISMATCH");
  return expected;
}

export function isVerifiedWesternSameArtifactCandidate(value) {
  return value !== null && typeof value === "object" && CANDIDATE_BRAND.has(value) && Object.isFrozen(value);
}

export function summarizeWesternSameArtifactCandidate(candidate) {
  if (!isVerifiedWesternSameArtifactCandidate(candidate)) fail("CANDIDATE_PRIVATE_BRAND_REQUIRED");
  return deepFreeze({
    ok: true,
...
export const westernSameArtifactTestOnly = deepFreeze({
  captureJsonData,
  canonicalCompact,
  parseStrictJsonBytes,
  verifyWesternE2EAssertionContractBindings,
