...
import { renderVedicSameArtifactCandidate } from
  "./render-vedic-civil-time-same-artifact-browser-observation-candidate.mjs";
import {
  VedicSameArtifactObservationError,
  collectVedicAuthoredBuildSourceSnapshot,
  collectVedicEvidenceToolSnapshot,
  collectVedicFormalContextSnapshot,
  collectVedicLockedBuildInputSnapshot,
  collectVedicOutputTree,
  deriveVedicEvidenceOnlyOutputBodyProbePaths,
  deriveVedicRequiredRuntimePaths,
  deriveVedicScenarioRuntimePaths,
  inspectVedicE2EAssertionContract,
  verifyVedicBuildManifest,
...
const PROJECT_HEADER = "x-hakimi-vedic-evidence-project";
const OUTPUT_TREE_HEADER = "x-hakimi-vedic-output-tree-sha256";
const IANA_PROBE_QUERY = "?hakimi-vedic-evidence-only=1";
...
function createOwnedTemporaryRoot() {
  const root = path.join(originalSystemTempRoot, `hakimi-vedic-same-artifact-${randomUUID()}`);
...
async function startStaticEvidenceServer(outDir, outputTree) {
  ...
  const requiredRuntimePaths = deriveVedicRequiredRuntimePaths(outputTree);
  const evidenceOnlyPaths = deriveVedicEvidenceOnlyOutputBodyProbePaths(outputTree);
  ...
  return Object.freeze({
    ...
    summarize() {
      ...
      const byProject = PROJECTS.map((definition) => {
        ...
        if (observed.browserDefaultFaviconResponseCount !== 1) {
          failLedger("per_browser_default_favicon_response_count", definition.projectName,
            1, observed.browserDefaultFaviconResponseCount);
        }
        return Object.freeze({
          ...
          scenarioRuntimeBodyManifestDigest:
            vedicSameArtifactTestOnly.servedBodyManifestDigest(identityOnly(runtimeFiles)),
          scenarioRuntimeResponseLedgerDigest:
            vedicSameArtifactTestOnly.servedResponseLedgerDigest(runtimeFiles),
          ...
          evidenceOnlyProbeBodyManifestDigest:
            vedicSameArtifactTestOnly.servedBodyManifestDigest(identityOnly(probeFiles)),
          evidenceOnlyProbeResponseLedgerDigest:
            vedicSameArtifactTestOnly.servedResponseLedgerDigest(probeFiles),
          combinedServedBodyManifestDigest:
            vedicSameArtifactTestOnly.servedBodyManifestDigest(identityOnly(combined)),
          combinedServedResponseLedgerDigest:
            vedicSameArtifactTestOnly.servedResponseLedgerDigest(combined)
        });
      });
      ...
      if (faviconAggregateResponseCount !== 2) {
        failLedger("aggregate_default_favicon_response_count", null,
          2, faviconAggregateResponseCount);
      }
...
function parseSanitizedSummary(summaryPath) {
  ...
  return vedicSameArtifactTestOnly.parseStrictJsonBytes(bytes, "sanitized Playwright sidecar");
}
...
export function parseFixedMatrixFailureSummaryBytes(bytes) {
  ...
  const value = vedicSameArtifactTestOnly.parseStrictJsonBytes(bytes, "fixed matrix failure sidecar");
  if (!exactObjectKeys(value, [
    "configuredProjectNames", "outcomeCount", "outcomes", "overallStatus", "schemaVersion"
  ]) || value.schemaVersion !== "hakimi.vedic.same-artifact-playwright-summary/1"
...
function exactTransformedModuleCount(buildOutput) {
  ...
  if (count !== 10) throw new Error("Vite transformed module count drifted");
  return count;
}
...
function temporaryCandidateOutputPath() {
  const candidate = path.join(
    ...
    `hakimi-vedic-same-artifact-candidate-${randomUUID()}.json`
  );
...
async function main() {
  ...
  try {
    const outLeaf = `hakimi-vedic-facts-${randomUUID().replaceAll("-", "")}`;
    ...
    const environment = isolatedEnvironment(ownedRoot, {
      HAKIMI_VEDIC_EVIDENCE_TEMP_ROOT: ownedRoot.root,
      HAKIMI_VEDIC_EVIDENCE_OUT_DIR: outDir,
      HAKIMI_VEDIC_EVIDENCE_CACHE_DIR: cacheDir,
      HAKIMI_VEDIC_FACT_ONLY_OUT_DIR: outDir
    });

    failureStageCode = "ASSERTION_CONTRACT_CHECK_FAILED_CLOSED";
    inspectVedicE2EAssertionContract(workspaceRoot);
    failureStageCode = "FORMAL_CONTEXT_CHECK_FAILED_CLOSED";
    collectVedicFormalContextSnapshot(workspaceRoot);
    ...
    const sourceBefore = collectVedicAuthoredBuildSourceSnapshot(workspaceRoot);
    const toolsBefore = collectVedicEvidenceToolSnapshot(workspaceRoot);
    const lockedBefore = collectVedicLockedBuildInputSnapshot(workspaceRoot);
    ...
    const outputTreeBefore = collectVedicOutputTree(outDir);
    verifyVedicBuildManifest(outDir, outputTreeBefore, workspaceRoot);
    const ianaEvidencePath = deriveVedicEvidenceOnlyOutputBodyProbePaths(outputTreeBefore)[0];
    ...
    const testEnvironment = isolatedEnvironment(ownedRoot, {
      ...
      HAKIMI_VEDIC_EVIDENCE_CACHE_DIR: cacheDir,
      HAKIMI_VEDIC_FACT_ONLY_OUT_DIR: outDir,
      HAKIMI_VEDIC_EVIDENCE_BASE_URL: evidenceServer.origin,
      HAKIMI_VEDIC_EVIDENCE_OUTPUT_TREE_SHA256: outputTreeBefore.treeDigest,
      HAKIMI_VEDIC_EVIDENCE_IANA_PATH: ianaEvidencePath,
      HAKIMI_VEDIC_EVIDENCE_SUMMARY_PATH: summaryPath,
      HAKIMI_VEDIC_EVIDENCE_PLAYWRIGHT_OUT_DIR: playwrightOutDir,
      HAKIMI_VEDIC_EVIDENCE_CONFIG_PROBE: "0"
    });
    ...
    vedicSameArtifactTestOnly.normalizePlaywrightSummary(playwrightSummary);
    ...
    vedicSameArtifactTestOnly.normalizeServer(serverObservation, outputTreeBefore);
    ...
    const outputTreeAfter = collectVedicOutputTree(outDir);
    const sourceAfter = collectVedicAuthoredBuildSourceSnapshot(workspaceRoot);
    const toolsAfter = collectVedicEvidenceToolSnapshot(workspaceRoot);
    const lockedAfter = collectVedicLockedBuildInputSnapshot(workspaceRoot);
    if (activeChildren.size !== 0) throw new Error("runner-controlled child process remained active");
    ...
    const rendered = renderVedicSameArtifactCandidate(workspaceRoot, observation);
...
if (import.meta.main) {
  main().catch((error) => {
    process.stderr.write(`${JSON.stringify({
      ...
      failureScenarioId,
      failureStatus,
      validationCode: error instanceof VedicSameArtifactObservationError ? error.code : null,
