...
const OK_PREFIX =
  "FOUR_SYSTEM_CURRENT_STATUS_OBSERVATION_CHILD_V2_9_OK";
const FAILED_PREFIX =
...
function visiblePreloadRejection() {
  ...
  for (let index = 0; index < process.execArgv.length; index += 1) {
    ...
    for (
      ...
      if (isOptionOrEqualsForm(value, FORBIDDEN_EXEC_OPTIONS[optionIndex])) {
        return PRELOAD_REJECTED;
      }
    }
  }
...
export async function main() {
  ...
  const {
    ...
    "./four-system-current-status-observation-child-v2-9-lib.mjs"
  );
  const result =
    await loadFourSystemCurrentStatusObservationChildV29(projectRoot);
  if (!isVerifiedFourSystemCurrentStatusObservationChildV29(result)) {
    throw new Error("PRIVATE_BRAND_REQUIRED");
  }
  const summary =
    getFourSystemCurrentStatusObservationChildV29Summary(result);
