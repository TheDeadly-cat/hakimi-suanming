...
const OK_PREFIX = "FOUR_SYSTEM_CURRENT_STATUS_V2_12_OK";
const FAILED_PREFIX = "FOUR_SYSTEM_CURRENT_STATUS_V2_12_FAILED";
...
export async function main() {
  ...
  const {
    FourSystemCurrentStatusObservationChildV212Error,
  ...
  } = await import("./four-system-current-status-observation-child-v2-12-lib.mjs");
  const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
  try {
    const child = await loadFourSystemCurrentStatusObservationChildV212(root);
    if (!isVerifiedFourSystemCurrentStatusObservationChildV212(child)) {
      process.stderr.write(`${FAILED_PREFIX} PRIVATE_CHILD_BRAND_MISSING\n`);
      return 1;
    }
    process.stdout.write(
      `${OK_PREFIX} ${JSON.stringify(getFourSystemCurrentStatusObservationChildV212Summary(child))}\n`
    );
    return 0;
  } catch (error) {
    const code = error instanceof FourSystemCurrentStatusObservationChildV212Error
      ? error.code
      : "INTERNAL_FAILURE";
