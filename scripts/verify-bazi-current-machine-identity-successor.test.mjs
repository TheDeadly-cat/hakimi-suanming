...
import {
  BaziCurrentMachineIdentitySuccessorError,
  baziCurrentMachineIdentitySuccessorTestOnly as testOnly,
  buildBaziCurrentMachineIdentitySuccessor,
  computeBaziCurrentMachineIdentitySuccessorDigest,
  ...
  serializeBaziCurrentMachineIdentitySuccessor
...
const CLI = path.join(HERE, "verify-bazi-current-machine-identity-successor.mjs");
const ARTIFACT = path.join(ROOT, "content", "system-admission", "bazi-current-machine-identity-successor.v1.0.0.json");
...
function codeIs(expected) {
  return (error) => error instanceof BaziCurrentMachineIdentitySuccessorError && error.code === expected;
}

function reseal(value) {
  value.receiptDigest = computeBaziCurrentMachineIdentitySuccessorDigest(value);
  return value;
}
...
test("exact persisted loader alone grants the successor private brand", async () => {
  const built = await buildBaziCurrentMachineIdentitySuccessor(ROOT);
  const loaded = await loadBaziCurrentMachineIdentitySuccessor(ROOT);
  assert.deepEqual(built, loaded);
  assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(built), false);
  assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(loaded), true);
...
test("artifact raw bytes, SHA-256, self digest, machine digest and canonical bytes are frozen", async () => {
  ...
  assert.equal(parsed.receiptDigest, computeBaziCurrentMachineIdentitySuccessorDigest(parsed));
  ...
  assert.equal(bytes.toString("utf8"), serializeBaziCurrentMachineIdentitySuccessor(parsed));
});

test("createdAt is canonical UTC and does not exceed the fixed build upper bound", async () => {
  const { createdAt } = await loadBaziCurrentMachineIdentitySuccessor(ROOT);
...
test("report receipt and expert child must be exact full-loader private brands", async () => {
  ...
  const expert = await loadBaziExpertCurrentLineZeroInstanceObservationChild(ROOT);
...
test("ordered inventory is exactly 28 unique identities in first-seen component order", async () => {
  const current = (await loadBaziCurrentMachineIdentitySuccessor(ROOT)).currentMachineIdentity;
...
test("wrong drift cardinality, path or component cannot survive self resealing", async () => {
  ...
  for (let index = 0; index < cases.length; index += 1) {
    ...
    cases[index](forged);
    reseal(forged);
    assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(forged), false);
...
test("clone, exact clone reseal and report identity mutation never gain the private brand", async () => {
  ...
  reseal(forged);
  assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(exactClone), false);
  assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(forged), false);
  assert.throws(() => getBaziCurrentMachineIdentitySuccessorSummary(exactClone), codeIs("SUCCESSOR_BRAND_REQUIRED"));
});
...
test("captured WeakSet intrinsics resist post-import brand poisoning", async () => {
  ...
  try {
    ...
    assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(loaded), true);
    assert.equal(isVerifiedBaziCurrentMachineIdentitySuccessor(clone(loaded)), false);
  } finally {
...
test("post-import Object.isFrozen poison cannot produce a mutable branded result", async () => {
  ...
  try {
    ...
    loaded = await loadBaziCurrentMachineIdentitySuccessor(ROOT);
  } finally {
...
test("captured canonical and summary paths do not consult poisoned array map or iterator", async () => {
  ...
  try {
    ...
    summary = getBaziCurrentMachineIdentitySuccessorSummary(loaded);
    serialized = serializeBaziCurrentMachineIdentitySuccessor(loaded);
  } finally {
...
test("CLI emits one narrow calibrated summary", () => {
  ...
  assert.match(run.stdout, /^BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_OK \{/u);
...
test("CLI rejects operands and visible preload options without leaking failure detail", () => {
  ...
  assert.equal(operand.stderr, "BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_FAILED VERIFICATION_FAILED\n");
  ...
  assert.equal(preload.stderr, "BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_FAILED VERIFICATION_FAILED\n");
});
