...
const OK_PREFIX =
  "FOUR_SYSTEM_CURRENT_STATUS_OBSERVATION_CHILD_V2_8_OK";
const FAILED_PREFIX =
...
export async function main() {
  ...
  } = await import("./four-system-current-status-observation-child-v2-8-lib.mjs");
  const result =
    await loadFourSystemCurrentStatusObservationChildV28(projectRoot);
  if (!isVerifiedFourSystemCurrentStatusObservationChildV28(result)) {
    throw new Error("PRIVATE_BRAND_REQUIRED");
  }
  const summary =
    getFourSystemCurrentStatusObservationChildV28Summary(result);
