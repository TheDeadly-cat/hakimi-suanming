...
const OK_PREFIX =
  "FOUR_SYSTEM_CURRENT_STATUS_OBSERVATION_CHILD_V2_6_OK";
const FAILED_PREFIX =
...
export async function main() {
  ...
  } = await import("./four-system-current-status-observation-child-v2-6-lib.mjs");
  const result =
    await loadFourSystemCurrentStatusObservationChildV26(projectRoot);
  if (!isVerifiedFourSystemCurrentStatusObservationChildV26(result)) {
    throw new Error("PRIVATE_BRAND_REQUIRED");
  }
  const summary =
    getFourSystemCurrentStatusObservationChildV26Summary(result);
