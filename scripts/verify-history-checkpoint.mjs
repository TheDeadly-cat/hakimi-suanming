...
const OK_PREFIX = "HISTORY_CHECKPOINT_OK";
const FAILED_PREFIX = "HISTORY_CHECKPOINT_FAILED";
...
export async function main() {
  ...
  try {
    const checkpoint = await loadHistoryCheckpoint(workspaceRoot);
    if (!isVerifiedHistoryCheckpoint(checkpoint)) {
      throw new HistoryCheckpointError(
        "CHECKPOINT_FULL_BRAND_REQUIRED",
        "history checkpoint 缺少 full-history 私有品牌。"
      );
    }
    process.stdout.write(
      `${OK_PREFIX} ${JSON.stringify(getHistoryCheckpointSummary(checkpoint))}\n`
    );
    return 0;
  } catch (error) {
    const code = error instanceof HistoryCheckpointError
      ? error.code
      : "INTERNAL_FAILURE";
