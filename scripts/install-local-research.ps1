...
if ([string]::IsNullOrWhiteSpace($InstallRoot)) { $InstallRoot = Join-Path $env:LOCALAPPDATA "HakimiBaziWorkbench" }
...
$targetRoot = Join-Path ([IO.Path]::GetFullPath($InstallRoot)) "packages\c15ef05bb165"
$cliPath = Join-Path $sourceRoot "scripts\local-research.mjs"
if (-not (Test-Path -LiteralPath $cliPath -PathType Leaf)) { throw "Extract the fixed local package first and pass -PackageRoot. Source checkout installation does not build an artifact." }
...
$launcher = Join-Path $targetRoot "scripts\launch-local-research.ps1"
$powershellPath = (Get-Command powershell.exe -ErrorAction Stop).Source
$desktopPath = [Environment]::GetFolderPath([Environment+SpecialFolder]::Desktop)
$title = (-join ([char[]]@(0x54C8,0x57FA,0x7C73,0x516B,0x5B57,0x7814,0x7A76,0x53F0))) + " (5188)"
...
if (Test-Path -LiteralPath $shortcutPath) {
  ...
  $shortcut.Description = "Fixed local research artifact on 127.0.0.1:5188; no build or automatic data migration."
  $shortcut.Save()
}
