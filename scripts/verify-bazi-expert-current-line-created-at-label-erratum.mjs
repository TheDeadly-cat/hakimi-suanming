...
try {
  ...
  } = await import("./bazi-expert-current-line-created-at-label-erratum-lib.mjs");
  const verified = await loadBaziExpertCurrentLineCreatedAtLabelErratum(PROJECT_ROOT);
  if (!isVerifiedBaziExpertCurrentLineCreatedAtLabelErratum(verified)) fail("PRIVATE_BRAND_REQUIRED");
  process.stdout.write(
    "BAZI_EXPERT_CURRENT_LINE_CREATED_AT_LABEL_ERRATUM_OK "
      + JSON.stringify(getBaziExpertCurrentLineCreatedAtLabelErratumSummary(verified))
      + "\n"
  );
} catch (reason) {
  process.stderr.write(JSON.stringify({
    errorCode: typeof reason?.code === "string" ? reason.code : "VERIFICATION_FAILED",
    message: reason instanceof Error ? reason.message : "Erratum verification failed."
  }) + "\n");
  process.exitCode = 1;
}
