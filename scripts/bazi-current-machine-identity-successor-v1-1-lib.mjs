...
export const BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_V1_1_RELATIVE_PATH =
  "content/system-admission/bazi-current-machine-identity-successor.v1.1.0.json";

const SUCCESSOR_ID = "hakimi.bazi.current-machine-identity-successor/1.1.0";
const RECORD_TYPE = "bazi_current_machine_identity_successor_v1_1";
const CREATED_AT = "2026-09-01T13:52:00.000Z";
const CREATED_AT_UPPER_BOUND = "2026-09-01T13:52:10.982Z";
const RECEIPT_DIGEST_DOMAIN = "hakimi.bazi.current-machine-identity-successor.receipt/1.1.0";
const MACHINE_IDENTITY_DIGEST_DOMAIN = "hakimi.bazi.current-machine-identity/1.0.0";
const HISTORICAL_RECEIPT_DIGEST_DOMAIN =
  "hakimi.bazi.current-machine-identity-successor.receipt/1.0.0";
const HISTORICAL_SUCCESSOR_V1 = OBJECT_FREEZE({
  ...
  observedCreatedAtLabel: "2026-09-01T08:10:46.766Z"
});
...
const EXPERT_CHILD = OBJECT_FREEZE({
  path: BAZI_EXPERT_CURRENT_LINE_ZERO_INSTANCE_OBSERVATION_CHILD_V1_1_RELATIVE_PATH,
  rawBytes: 9_765,
...
export class BaziCurrentMachineIdentitySuccessorV11Error extends Error {
  constructor(code, message, options) {
    super(`${code}: ${message}`, options);
    this.name = "BaziCurrentMachineIdentitySuccessorV11Error";
    this.code = code;
  }
}

function fail(code, message, options) {
  throw new BaziCurrentMachineIdentitySuccessorV11Error(code, message, options);
}
...
export function computeBaziCurrentMachineIdentitySuccessorV11Digest(value) {
  return sha256Text(`${RECEIPT_DIGEST_DOMAIN}\0${canonicalStringify(unsignedReceipt(value))}`);
}

function computeHistoricalSuccessorV1Digest(value) {
  return sha256Text(`${HISTORICAL_RECEIPT_DIGEST_DOMAIN}\0${canonicalStringify(unsignedReceipt(value))}`);
}

export function serializeBaziCurrentMachineIdentitySuccessorV11(value) {
...
async function readHistoricalSuccessorV1(workspaceRoot) {
  const snapshot = await readBaziDttStableWorkspaceArtifact(workspaceRoot, HISTORICAL_SUCCESSOR_V1.path);
  assertSnapshot(snapshot, HISTORICAL_SUCCESSOR_V1, "HISTORICAL_SUCCESSOR_V1_RAW_DRIFT");
  const predecessor = parseBaziDttStrictJsonArtifact(snapshot);
  if (predecessor?.successorId !== HISTORICAL_SUCCESSOR_V1.successorId
    || predecessor?.receiptDigest !== HISTORICAL_SUCCESSOR_V1.receiptDigest
    || computeHistoricalSuccessorV1Digest(predecessor) !== HISTORICAL_SUCCESSOR_V1.receiptDigest
    || predecessor?.createdAt !== HISTORICAL_SUCCESSOR_V1.observedCreatedAtLabel
    || predecessor?.currentMachineIdentity?.currentMachineIdentityDigest
      !== EXPECTED_CURRENT_MACHINE_IDENTITY_DIGEST
    || predecessor?.currentMachineIdentity?.orderedUniqueFileIdentityCount !== 28
    || predecessor?.historicalFormalBaseline?.currentManifestClaimed !== false
    || predecessor?.expertOverlay?.includedInCurrentMachineIdentityDigest !== false
    || predecessor?.observationBoundary?.persistedAsDomainManifest !== false
    || predecessor?.observationBoundary?.crossFileAtomicSnapshot !== false
    || predecessor?.observationBoundary?.mutationEpochReceipt !== null
    || predecessor?.authorityBoundary?.contentTruthEstablished !== false
    || predecessor?.authorityBoundary?.expertTruthEstablished !== false
    || predecessor?.authorityBoundary?.rightsLegalConclusionEstablished !== false
    || predecessor?.authorityBoundary?.releaseReady !== false
    || predecessor?.authorityBoundary?.publicDeploymentAuthorized !== false
    || predecessor?.authorityBoundary?.expertClaimsAuthorized !== false
    || predecessor?.releaseGovernance?.releaseIdentity !== "legacy-v13"
    || predecessor?.releaseGovernance?.targetSchema !== 13
    || predecessor?.releaseGovernance?.migrationId !== null) {
    fail(
      "HISTORICAL_SUCCESSOR_V1_SELF_OR_RED_SEMANTIC_DRIFT",
      "旧 successor v1 只能作 exact raw+self 且全红的历史前件。"
    );
  }
  return deepFreeze({
    ...rawPin(HISTORICAL_SUCCESSOR_V1),
    successorId: HISTORICAL_SUCCESSOR_V1.successorId,
    receiptDigest: HISTORICAL_SUCCESSOR_V1.receiptDigest,
    observedCreatedAtLabel: HISTORICAL_SUCCESSOR_V1.observedCreatedAtLabel,
...
function assertExpertChild(child) {
  if (!isVerifiedBaziExpertCurrentLineZeroInstanceObservationChildV11(child)) {
  ...
  if (child?.childId !== EXPERT_CHILD.childId || child?.childDigest !== EXPERT_CHILD.childDigest
    ...
    || child?.manifestAndEndpointBoundary?.currentFullDomainManifestEstablished !== false
    || child?.manifestAndEndpointBoundary?.formalManifestModified !== false
    || child?.manifestAndEndpointBoundary?.centralRegistryModified !== false
    || child?.manifestAndEndpointBoundary?.globalCurrentEndpointRegistrationModified !== false
    || child?.manifestAndEndpointBoundary?.thisChildReplacesBaziMachineIdentityEndpoint !== false
    || child?.manifestAndEndpointBoundary?.thisChildReplacesFourSystemStatusEndpoint !== false
    || child?.currentRepositoryZeroInstanceProjection?.identitiesVerified !== 0
    || child?.currentRepositoryZeroInstanceProjection?.credentialsVerified !== 0
    || child?.currentRepositoryZeroInstanceProjection?.scopesVerified !== 0
    || child?.currentRepositoryZeroInstanceProjection?.pairwiseIndependenceAssessmentsVerified !== 0
    || child?.currentRepositoryZeroInstanceProjection?.originalOpinionsVerified !== 0
    || child?.currentRepositoryZeroInstanceProjection?.independentExpertReviewsVerified !== 0
    ...
    || child?.materialAccessBoundary?.piiBearingArtifactsReadByThisChild !== 0
    || child?.materialAccessBoundary?.privateDossierArtifactsReadByThisChild !== 0
    || child?.materialAccessBoundary?.privateOpinionFilesReadByThisChild !== 0
...
async function loadCurrentParents(workspaceRoot) {
  ...
  const expertChild = await loadBaziExpertCurrentLineZeroInstanceObservationChildV11(workspaceRoot);
...
function buildSuccessor(formalSnapshot, formal, reportReceipt, expertChild, historicalPredecessor, observation) {
  const successor = {
    schemaVersion: "1.1.0",
    ...
    historicalPredecessorV1: historicalPredecessor,
    ...
    timeBoundary: {
      ...
      predecessorV1ChronologyAuthorityEstablished: false
    },
    ...
    authorityBoundary: {
      bindingClosureEstablished: false,
      sourceBindingClosureEstablished: false,
      sourceRightsClosureEstablished: false,
      contentTruthEstablished: false,
      expertTruthEstablished: false,
      rightsLegalConclusionEstablished: false,
      browserRuntimeEvidenceEstablished: false,
      runtimeIdentityEstablished: false,
      trustedTimeEstablished: false,
      mutationEpochEstablished: false,
      formalCurrentManifestEstablished: false,
    ...
    lineage: {
      ...
      historicalPredecessorModified: false,
      historicalPredecessorBacklinkAdded: false,
      ...
      thisSuccessorReplacesCurrentEndpoint: false,
      thisSuccessorReplacesFourSystemStatusEndpoint: false,
  ...
  successor.receiptDigest = computeBaziCurrentMachineIdentitySuccessorV11Digest(successor);
  return deepFreeze(successor);
}

export async function buildBaziCurrentMachineIdentitySuccessorV11(
  workspaceRoot = REFLECT_APPLY(PROCESS_CWD, process, [])
) {
  const { reportReceipt, expertChild } = await loadCurrentParents(workspaceRoot);
  const historicalPredecessor = await readHistoricalSuccessorV1(workspaceRoot);
  ...
  return buildSuccessor(formalSnapshot, formal, reportReceipt, expertChild, historicalPredecessor, observation);
}
...
function assertPersistedSemantic(persisted, expected) {
  if (persisted?.successorId !== SUCCESSOR_ID || persisted?.recordType !== RECORD_TYPE
    ...
    || persisted?.timeBoundary?.createdAtUpperBoundObservedOnSameUntrustedLocalClock !== CREATED_AT_UPPER_BOUND
    || persisted.createdAt >= persisted.timeBoundary.createdAtUpperBoundObservedOnSameUntrustedLocalClock
    || persisted?.timeBoundary?.trustedTimestampEstablished !== false
    || persisted?.timeBoundary?.externalTimeAuthorityEstablished !== false
    || persisted?.timeBoundary?.crossArtifactTemporalOrderEstablished !== false
    || persisted?.timeBoundary?.predecessorV1CorrectedCreatedAt !== null
    || persisted?.historicalPredecessorV1?.successorId !== HISTORICAL_SUCCESSOR_V1.successorId
    || persisted?.historicalPredecessorV1?.receiptDigest !== HISTORICAL_SUCCESSOR_V1.receiptDigest
    || persisted?.historicalPredecessorV1?.rawAndSelfDigestVerified !== true
    || persisted?.historicalPredecessorV1?.fullLoaderImportedByThisSuccessor !== false
    || persisted?.historicalPredecessorV1?.fullLoaderInvokedByThisSuccessor !== false
    || persisted?.historicalPredecessorV1?.privateBrandConsumed !== false
    || persisted?.historicalPredecessorV1?.brandCurrent !== false
    || persisted?.receiptDigest !== EXPECTED_PERSISTED.receiptDigest
    || !SHA256.test(persisted?.receiptDigest)
    || computeBaziCurrentMachineIdentitySuccessorV11Digest(persisted) !== persisted.receiptDigest
    ...
    || persisted?.expertOverlay?.includedInCurrentMachineIdentityDigest !== false
    || persisted?.historicalFormalBaseline?.formalManifestStillHistorical !== true
    || persisted?.historicalFormalBaseline?.formalManifestCurrent !== false
    ...
    || persisted?.authorityBoundary?.bindingClosureEstablished !== false
    || persisted?.authorityBoundary?.sourceBindingClosureEstablished !== false
    || persisted?.authorityBoundary?.sourceRightsClosureEstablished !== false
    || persisted?.authorityBoundary?.contentTruthEstablished !== false
    || persisted?.authorityBoundary?.expertTruthEstablished !== false
    || persisted?.authorityBoundary?.rightsLegalConclusionEstablished !== false
    || persisted?.authorityBoundary?.runtimeIdentityEstablished !== false
    || persisted?.authorityBoundary?.trustedTimeEstablished !== false
    || persisted?.authorityBoundary?.mutationEpochEstablished !== false
    || persisted?.authorityBoundary?.formalCurrentManifestEstablished !== false
    || persisted?.authorityBoundary?.formalAdmissionAuthorized !== false
    || persisted?.authorityBoundary?.publicReleaseAuthorized !== false
    ...
    || persisted?.lineage?.formalManifestModified !== false
    || persisted?.lineage?.centralRegistryModified !== false
    || persisted?.lineage?.globalCurrentEndpointRegistrationModified !== false
    || persisted?.lineage?.thisSuccessorReplacesCurrentEndpoint !== false
    || persisted?.lineage?.thisSuccessorReplacesFourSystemStatusEndpoint !== false
    || !exactJson(persisted, expected)) {
    fail("SUCCESSOR_CURRENT_MISMATCH", "持久化 successor 不等于当前唯一机械投影或红门被抬升。");
  }
}

export async function loadBaziCurrentMachineIdentitySuccessorV11(
  workspaceRoot = REFLECT_APPLY(PROCESS_CWD, process, [])
) {
  ...
  const expected = await buildBaziCurrentMachineIdentitySuccessorV11(workspaceRoot);
  const snapshot = await readBaziDttStableWorkspaceArtifact(
    workspaceRoot,
    BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_V1_1_RELATIVE_PATH
  );
  assertSnapshot(snapshot, { path: BAZI_CURRENT_MACHINE_IDENTITY_SUCCESSOR_V1_1_RELATIVE_PATH, ...EXPECTED_PERSISTED }, "SUCCESSOR_RAW_DRIFT");
  const persisted = parseBaziDttStrictJsonArtifact(snapshot);
  if (decodeSnapshot(snapshot) !== serializeBaziCurrentMachineIdentitySuccessorV11(persisted)) {
...
export function isVerifiedBaziCurrentMachineIdentitySuccessorV11(value) {
  return value !== null && typeof value === "object" && REFLECT_APPLY(WEAK_SET_HAS, VERIFIED_RESULTS, [value]);
}

export function getBaziCurrentMachineIdentitySuccessorV11Summary(value) {
  if (!isVerifiedBaziCurrentMachineIdentitySuccessorV11(value)) {
  ...
  return deepFreeze({
    ...
    currentExpertChildId: value.expertOverlay.childId,
    historicalPredecessorRawSelfVerified: value.historicalPredecessorV1.rawAndSelfDigestVerified,
    oldV1FullLoaderImported: value.historicalPredecessorV1.fullLoaderImportedByThisSuccessor,
    oldV1FullLoaderInvoked: value.historicalPredecessorV1.fullLoaderInvokedByThisSuccessor,
    ...
    formalManifestCurrent: value.historicalFormalBaseline.formalManifestCurrent,
    replacesCurrentEndpoint: value.lineage.thisSuccessorReplacesCurrentEndpoint,
...
export const baziCurrentMachineIdentitySuccessorV11TestOnly = OBJECT_FREEZE({
  SUCCESSOR_ID,
  RECORD_TYPE,
  RECEIPT_DIGEST_DOMAIN,
  MACHINE_IDENTITY_DIGEST_DOMAIN,
  HISTORICAL_RECEIPT_DIGEST_DOMAIN,
  CREATED_AT,
  CREATED_AT_UPPER_BOUND,
  HISTORICAL_SUCCESSOR_V1,
  FORMAL_V22,
  REPORT_RECEIPT,
  EXPERT_CHILD,
  REPORT_COMPONENT,
