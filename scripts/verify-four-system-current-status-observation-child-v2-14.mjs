...
const OK_PREFIX = "FOUR_SYSTEM_CURRENT_STATUS_V2_14_OK";
const FAILED_PREFIX = "FOUR_SYSTEM_CURRENT_STATUS_V2_14_FAILED";
...
export async function main() {
  ...
  const {
    FourSystemCurrentStatusObservationChildV214Error,
  ...
  } = await import("./four-system-current-status-observation-child-v2-14-lib.mjs");
  const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
  try {
    const child = await loadFourSystemCurrentStatusObservationChildV214(root);
    if (!isVerifiedFourSystemCurrentStatusObservationChildV214(child)) {
      process.stderr.write(`${FAILED_PREFIX} PRIVATE_CHILD_BRAND_MISSING\n`);
      return 1;
    }
    process.stdout.write(
      `${OK_PREFIX} ${JSON.stringify(
        getFourSystemCurrentStatusObservationChildV214Summary(child)
      )}\n`
    );
    return 0;
  } catch (error) {
    const code = error instanceof FourSystemCurrentStatusObservationChildV214Error
      ? error.code
      : "INTERNAL_FAILURE";
