...
function visibleUnsafeNodeLaunchState() {
  ...
  return { execArgv: [...process.execArgv], inherited };
}
...
async function main() {
  if (process.argv.length !== 2) {
    fail(
      ...
      "本验证器只读取固定工作区与固定候选路径，不接受 caller operand。"
    );
  }
  ...
  const result = await loadWesternProductizationVersionAwareObservationCandidate(workspaceRoot);
  if (!isVerifiedWesternProductizationVersionAwareObservationResult(result)) {
    fail("CURRENT_BRAND_MISSING", "固定路径 loader 没有签发同模块私有机械观察品牌。");
  }
