...
`97fbf61` 的 Quick CI `34712857692` 发布证据组为 712/713。唯一失败中，生成端记录 Edge `not-detected`，核验端随后读到 `Microsoft Edge 152.0.4191.66`。旧检测器没有保存子进程状态，不能从这条日志断定是超时、退出失败还是其他原因。

受控测试确认旧实现把超时、非零退出和空响应都降为 `not-detected`，并把 stderr 诊断串拼进版本。后续检测器只把 PATH 查找的 ENOENT 或确实不存在的 Windows 安装路径视为缺席；已找到的探测命令失败时抛出 `RELEASE_BROWSER_VERSION_PROBE_FAILED`，保留浏览器、可执行文件名、退出码、信号、错误码、stdout 是否为空及耗时。成功版本只取 stdout。探测失败不能再通过另一个别名掩盖，生成与复核仍各自读取实际版本，版本变化仍被原工具链比对拒绝。

本次没有把超时时间从 5 秒放宽，没有自动重试，也没有改签原证据。新增两项测试先在原实现上失败，再在修复后通过，覆盖缺席、ETIMEDOUT、EACCES、非零退出、信号终止、空响应、仅 stderr、重新读取版本和别名掩盖。它们使用受控子进程结果，不代表真实浏览器验收。

完整发布证据组为 27 文件、715/715 通过，零跳过、取消或缺失文件结果；其中主文件 142/142，目录隔离文件 12/12。另行 CI 合同为 56/56。当前本机元数据探测为 Edge 153.0.4234.32、Chrome 153.0.8010.36，这不是重新执行 PWA 浏览器测试；上文 c15ef 回执仍属于原浏览器版本。

原始受控失败、完整文件和整组日志保存在 Z 盘同一父目录的 `browser-version-probe-v1/`。首次远端 Edge 未识别的底层原因尚未确认，新提交仍需取得远端整组结果，不能用此次本地通过替代。
