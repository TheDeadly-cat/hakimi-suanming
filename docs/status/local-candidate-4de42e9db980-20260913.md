...
浏览器回执属于打包前的锁定应用。修复包安装副本的浏览器复验尚未执行，不能用上述回执替代这一步。
...
仍待：修复安装副本的浏览器验证、用户明确决定是否切换 5189、本人真实研究反馈、第二台 Windows 验证，以及剩余治理根因处理。真实专家资格仍为 0/2；缺失历史原文件、逐字校勘、底本关系与权利审阅继续独立处理。公开发布及旧版本迁移/回滚不是本次 GitHub 同步的完成声明。
