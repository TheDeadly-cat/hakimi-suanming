...
> 状态：默认发布身份仍为 `legacy-v13`；2026-09-23/24 重新区分历史自动接管、当前拒绝与新页面路径。第 5 节的 2026-08 浏览器数字均为历史记录，不转签给当前源码。当前 v13→v16 本地诊断运行是 24/26，两个浏览器的万条慢审计收敛场景均失败。
>
> 日期：2026-09-24（更新）；2026-08-10 为首版
...
### 1.1 当前测试路径与接管范围（2026-09-23）

下表区分当前代码允许的接管、历史数据库格式与工程候选；它不修改默认 v13 或已安装产物，也不把旧浏览器记录转签给新的检出。

| 路径 | 当前代码的范围 | 应取得的独立结果 |
| --- | --- | --- |
| v13 同 Schema A→B | 数据库描述符相同的正常本地更新 | 原双代际协议测试；不以跨 Schema 结果替代 |
| v13→v16 | 显式工程候选允许前向接管；仍非默认发布 | Edge/Chrome 各 13 项的完整当前矩阵 |
| v13→v14 | 不允许旧页自动前向接管；新页面直接启动及 v14 物化是另外的路径 | 原历史矩阵逐项对齐，另验证拒绝接管和源数据不变 |
| v13→v15 | 不允许旧页自动前向接管；保留历史格式和新页面路径的适用范围 | 原历史矩阵逐项对齐，不能强制 worker 激活 |
| v14→v15 | 不属于当前 legacy-v13→v16 自动接管谱系 | 历史兼容与当前拒绝分开记录 |
| 孤立 v13 恢复 | 没有旧壳、精确单一 v13 库时只读救援 | v15/v16 两个目标分别在 Edge/Chrome 运行，共 4 项 |

历史组的容量不足、事务中止、目标校验/摘要、并发写入、BOOT_OK 中断和陈旧连接场景，分别已有当前 v13→v16 矩阵的具名对应测试；只有该组实际完整执行，才可报告这些场景的当前结果。v14/v15 的独有索引、收据及新页面行为仍须按其自身断言验证。旧页拒绝接管不证明这些后续场景已经执行。

迁移 CI 将五个现有组拆成独立矩阵任务，最多两个并行且不因其他组失败而取消。每组只上传自己的隔离诊断目录：JSON 全量结果、失败截图/trace/错误上下文、构建身份，以及 requested head/base、workflow SHA 和实际 checkout/parents。默认 PR checkout 的合并预览与历史工作流的精确 PR head 分别记录。聚合任务仍要求五组全部成功；本节不是新增通过声明。

### 1.2 历史断言的保留与当前覆盖

三个原始 `service-worker-cross-schema-{upgrade,v13-v15,v14-v15}.spec.ts` 文件逐字节保留在本轮基线 `93f91d4` 的状态。它们描述旧接管条件，当前常规发现排除这三个文件；三个专用配置改选 `historical-{v13-v14,v13-v15,v14-v15}-boundary.spec.ts`，不会执行旧 v15 文件里的强制激活辅助函数。新套件通过不代表原 8/20/18 项通过。

| 历史承诺 | 当前具名验证位置与范围 |
| --- | --- |
| 旧页面自动跨代接管 | 三条 boundary 路径各检查 `PROTOCOL_MISMATCH`、源库及控制记录不变、目标未创建；这是拒绝验证 |
| 新页面容量准入、索引及持久提交 | 三条 boundary 路径的“新页面直接启动”；观察真实准入信号，并验证 committed/BOOT_OK 和数据库内容 |
| 历史格式无损物化、活动流索引、旧记录不补收据 | 三条 boundary 路径的“所有旧页关闭后由浏览器自然激活”；源行逐项比较，v15 新建 Revision 才产生一条基线收据 |
| 富数据及并发旧页写锁 | 当前 v16 spec 的“富 v13 数据直升 v16…”及“v16 冻结多个 v13 页的真实写入…” |
| 容量不足、启动校验失败、摘要不符 | 当前 v16 spec 中三个同名失败路径；分别验证源保留、目标零创建或隔离和控制指针 |
| 真实事务中止、BOOT_OK 中断、陈旧连接阻塞 | 当前 v16 spec 中三个同名失败路径；不以旧接管前的超时代替这些断言 |
| 同 ID 只清理、新 ID 长期重发 | 当前 v16 spec 的“v16 隔离受阻后同 migrationId 只清理不续跑…” |
| dirty epoch 与 CAS 并发保护 | 当前 v16 spec 的“dirty v16 全审计期间并发受支持写入使 CAS 失败…” |
| 无旧壳 v13 只读恢复与 ZIP 预检 | 独立 orphaned-v13-recovery 组；目标 v15/v16 × Edge/Chrome 四项 |
| v15 缺失 control 时旧壳重建、成功用户新 ID 壳沿用旧谱系、迁移后完整备份再恢复 | 保留原 v15 历史断言及第 5 节原范围；新三项 boundary 套件没有覆盖这些完整组合，不将其称为当前浏览器通过 |

2026-09-24 本地三条 boundary 运行分别为 v13→v14 Edge 3/3、v13→v15 双浏览器 6/6、v14→v15 双浏览器 6/6；恢复组修复手机装饰层溢出及反馈选择器歧义后 4/4。v16 的本地诊断运行 24/26，两个失败均为首项万条慢审计收敛；已有 JSON、失败 trace、截图和控制状态，故没有扩大重试或修改接管权限。这些是分支工作树的本地运行，最终 PR 的实际合并检出与结果必须另看 CI 记录。
...
这些结果只证明当时的代码与固定浏览器环境中的工程复现；不把 v16 提升为默认发布代，不等于整个 Web v1、Firefox 或 Android 已通过，也不构成命理专家真值。
...
2026-08 的 v16 26/26、Web v1 2/2 与 clean 万条容量记录以及 v14/v15 数字均按原时间和输入保留；它们不能关闭第 1 节当前检出的失败或未覆盖组合。普通 `npm run build` 与 `dist/web` 的发布身份仍固定为 `legacy-v13`；这些工程证据不等于 v16 已成为默认生产代，不等于整个 Web v1、Android APK 或专家真值已经完成。
