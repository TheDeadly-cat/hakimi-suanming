...
function requiredOutputFile(environmentName: string): string {
  ...
  if (!parentStat.isDirectory() || parentStat.isSymbolicLink()
      || !samePath(parent, realParent) || relative.length === 0
...
const baseURL = process.env.HAKIMI_VEDIC_EVIDENCE_BASE_URL;
if (typeof baseURL !== "string" || !/^http:\/\/127\.0\.0\.1:\d{1,5}$/u.test(baseURL)) {
  throw new Error("HAKIMI_VEDIC_EVIDENCE_BASE_URL must be an exact loopback HTTP origin");
}
...
if (typeof outputTreeDigest !== "string" || !/^[a-f0-9]{64}$/u.test(outputTreeDigest)) {
  throw new Error("HAKIMI_VEDIC_EVIDENCE_OUTPUT_TREE_SHA256 must be SHA-256");
}
...
if (typeof ianaEvidencePath !== "string"
  ...
  throw new Error("HAKIMI_VEDIC_EVIDENCE_IANA_PATH must be one exact retained-output path");
}

const configDirectory = path.dirname(fileURLToPath(import.meta.url));
const summaryPath = requiredOutputFile("HAKIMI_VEDIC_EVIDENCE_SUMMARY_PATH");
const outputDir = requiredChildDirectory("HAKIMI_VEDIC_EVIDENCE_PLAYWRIGHT_OUT_DIR");
const configProbe = process.env.HAKIMI_VEDIC_EVIDENCE_CONFIG_PROBE === "1";
  ...
  projects: [
    {
      ...
      use: {
        ...
        channel: "chrome",
        extraHTTPHeaders: { "x-hakimi-vedic-evidence-project": "chrome" }
      }
    },
    {
      name: "msedge",
      use: {
        ...
        channel: "msedge",
        extraHTTPHeaders: { "x-hakimi-vedic-evidence-project": "msedge" }
      }
    }
  ]
});
