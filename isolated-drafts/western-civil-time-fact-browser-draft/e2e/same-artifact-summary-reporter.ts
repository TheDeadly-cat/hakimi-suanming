...
const FIXED_STAGE_STATUS = new Map<string, SanitizedStatus>([
  ["WESTERN_FIXED_STAGE:bootstrap", "failed_stage_bootstrap"],
  ["WESTERN_FIXED_STAGE:after_each_problem_check_console_warning",
    "failed_stage_after_each_problem_check_console_warning"],
  ["WESTERN_FIXED_STAGE:after_each_problem_check_console_error",
    "failed_stage_after_each_problem_check_console_error"],
  ["WESTERN_FIXED_STAGE:after_each_problem_check_pageerror",
    "failed_stage_after_each_problem_check_pageerror"],
  ["WESTERN_FIXED_STAGE:after_each_problem_check_requestfailed",
    "failed_stage_after_each_problem_check_requestfailed"],
  ["WESTERN_FIXED_STAGE:unique_input", "failed_stage_unique_input"],
  ["WESTERN_FIXED_STAGE:unique_run_resolved", "failed_stage_unique_run_resolved"],
  ["WESTERN_FIXED_STAGE:unique_exact_facts", "failed_stage_unique_exact_facts"],
  ["WESTERN_FIXED_STAGE:unique_input_clear", "failed_stage_unique_input_clear"],
  ["WESTERN_FIXED_STAGE:unique_iana_navigation", "failed_stage_unique_iana_navigation"]
]);
...
function requiredEvidenceTempRoot(): string {
  ...
  if (typeof raw !== "string" || !path.isAbsolute(raw)) {
    throw new Error("Western evidence temp root must be absolute");
  }
  ...
  if (!/^hakimi-western-same-artifact-[a-f0-9-]{36}$/u.test(path.basename(resolved))
    ...
    throw new Error("Western evidence temp root must be the runner-owned isolated process temp or its real child");
  }
  return real;
}
...
function requiredTemporaryOutput(value: unknown, evidenceTempRoot: string): string {
  if (typeof value !== "string" || !path.isAbsolute(value)) {
    throw new Error("Western summaryPath must be absolute");
  }
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink() || !samePath(parent, realParent)
      || relative.length === 0 || relative.startsWith("..") || path.isAbsolute(relative)) {
    throw new Error("Western summaryPath parent must be below the runner-owned temp root");
  }
  return path.join(realParent, path.basename(resolved));
}

export default class WesternSameArtifactSummaryReporter implements Reporter {
  ...
  constructor(options: ReporterOptions = {}) {
    ...
    if (typeof options.configProbe !== "boolean") {
      throw new Error("Western configProbe must be an explicit boolean");
    }
  ...
  onBegin(config: FullConfig, suite: Suite): void {
    ...
    if (JSON.stringify(this.configuredProjectNames) !== JSON.stringify(PROJECT_NAMES)) {
      throw new Error("Western evidence suite requires exact chrome and msedge projects");
    }
    const declarations = new Set<string>();
    for (const currentTest of suite.allTests()) {
      const projectName = currentTest.parent.project()?.name ?? "";
      const scenarioId = SCENARIO_BY_TITLE.get(currentTest.title);
      const key = `${projectName}\u0000${scenarioId ?? ""}`;
      if (!PROJECT_NAMES.includes(projectName as "chrome" | "msedge") || !scenarioId
          || currentTest.expectedStatus !== "passed" || declarations.has(key)) {
        throw new Error("Western evidence suite contains an extra, duplicate, or non-passing declaration");
      }
      declarations.add(key);
    }
    this.declaredOutcomeCount = suite.allTests().length;
    if (this.declaredOutcomeCount !== 10 || declarations.size !== 10) {
      throw new Error("Western evidence suite must declare exactly ten outcomes");
    }
  }
  ...
  onTestEnd(currentTest: TestCase, result: TestResult): void {
    ...
    if (!PROJECT_NAMES.includes(projectName as "chrome" | "msedge") || !scenarioId
        || this.outcomeKeys.has(key) || currentTest.expectedStatus !== "passed" || result.retry !== 0
      ...
      throw new Error("Western evidence outcome is extra, duplicate, retried, or has an unknown fixed status");
    }
    this.outcomeKeys.add(key);
    this.outcomes.push(Object.freeze({ projectName, scenarioId, status, retry: 0 }));
  }
  ...
  onEnd(result: FullResult): void {
    if (this.configProbe) {
      if (result.status !== "passed" || this.declaredOutcomeCount !== 10
        ...
        throw new Error("Western config path probe did not remain list-only");
      }
      writeFileSync(this.summaryPath, `${JSON.stringify({
        schemaVersion: "hakimi.western.same-artifact-config-path-probe/1",
